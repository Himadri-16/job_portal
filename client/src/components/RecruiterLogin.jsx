  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if(state==="Login"){
        ...
        const {data} = await axios.post(backUrl+'/api/company/register',formData)
  ...
  return (
    <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {state === "Login" && <p className='text-sm text-red-600 mt-4 cursor-pointer'>Forgot Password?</p>}
        ...
        {
          state === 'Login'
            ? <p className='mt-5 text-center '>Don't have an account? <span className='text-red-600 cursor-pointer' onClick={() => setState("Sign Up")}>Sign Up</span></p>
